...
TEST(Task, reuse) {
	...
	try {
		configure(t);
		EXPECT_TRUE(t.plan(1) == SUCCESS_ERROR_CODE);
...
TEST(Task, timeout) {
	...
	EXPECT_TRUE(t.plan(3) == SUCCESS_ERROR_CODE);
	EXPECT_EQ(t.solutions().size(), 3u);

	// zero timeout fails
	t.reset();
	t.setTimeout(0.0);
	EXPECT_TRUE(t.plan().val == moveit_msgs::msg::MoveItErrorCodes::TIMED_OUT);

	// time for 1 solution
	t.reset();
	t.setTimeout(std::chrono::duration<double>(timeout).count());
	EXPECT_EQ(t.plan(), SUCCESS_ERROR_CODE);
